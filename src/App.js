...
import enhancedAlertManager from './Utils/enhancedAlertManager';
...
class App extends Component {
  ...
  addCoinz = (coin) => {
    ...
    if (!ticker || !costBasis || !hodl) {
      ...
      // Add notification for coin added
      const notificationManager = require('./Utils/notificationManager').default;
      notificationManager.addCoinAddedNotification(ticker.toUpperCase(), hodl);
  ...
  // Original market data fetching method (kept for compatibility)
  marketData = async (userCoinz) => {
    try {
      ...
      // Update alert monitoring with new market data
      alertMonitor.updateMarketData(marketData, this.state.exchangeRates[this.state.pref.currency] || 1);

      // Update portfolio data in enhanced alert manager
      this.updateAlertManagerPortfolio();
    } catch (e) {
  ...
  // Enhanced market data fetching with Binance integration
  fetchEnhancedMarketData = async (userCoinz) => {
    try {
      if (!userCoinz || Object.keys(userCoinz).length === 0) {
        this.setState({ marketData: {} });
        return;
      ...
      // Fetch Binance data for supported symbols
      const binanceSymbols = ['BTC', 'ETH', 'ADA', 'DOT', 'LINK', 'LTC', 'BCH', 'XRP', 'EOS', 'TRX'];
      const supportedTickers = userTickers.filter(ticker => binanceSymbols.includes(ticker.toUpperCase()));

      if (supportedTickers.length > 0) {
        try {
          const binanceSymbolsParam = supportedTickers.map(ticker => `"${ticker.toUpperCase()}USDT"`).join(',');
          const binanceUrl = `https://api.binance.com/api/v3/ticker/24hr?symbols=[${binanceSymbolsParam}]`;
          ...
          if (binanceResponse.ok) {
            ...
            binanceData.forEach(ticker => {
              ...
              if (userTickers.includes(symbol)) {
                marketData[symbol] = {
                  ticker: {
          ...
          console.warn('Failed to fetch Binance data:', binanceError);
        }
      }

      // Fetch remaining data from CoinGecko for unsupported symbols
      const remainingTickers = userTickers.filter(ticker => !supportedTickers.includes(ticker));

      if (remainingTickers.length > 0) {
        // Try CryptoCompare first for remaining tickers
        const cryptocompareSymbols = remainingTickers.filter(ticker =>
          ['BTC', 'ETH', 'ADA', 'DOT', 'LINK', 'LTC', 'BCH', 'XRP', 'EOS', 'TRX'].includes(ticker.toUpperCase())
        );

        if (cryptocompareSymbols.length > 0) {
          try {
            const cryptocompareUrl = `https://min-api.cryptocompare.com/data/pricemulti?fsyms=${cryptocompareSymbols.join(',')}&tsyms=USD&e=CCCAGG`;
            ...
            if (cryptocompareResponse.ok) {
              ...
              Object.entries(cryptocompareData).forEach(([symbol, priceData]) => {
                ...
                if (remainingTickers.includes(symbolLower)) {
                  marketData[symbolLower] = {
                    ticker: {
                      base: symbol,
                      target: 'USD',
                      price: priceData.USD
            ...
            console.warn('Failed to fetch CryptoCompare data:', cryptocompareError);
          }
        }

        // Fallback to CoinGecko for any remaining unsupported symbols
        const finalRemainingTickers = remainingTickers.filter(ticker => !marketData[ticker]);

        if (finalRemainingTickers.length > 0) {
          try {
            const listRes = await fetch("https://api.coingecko.com/api/v3/coins/list");
            if (!listRes.ok) throw new Error(`Coins list HTTP ${listRes.status}`);
            const allCoins = await listRes.json();
            const usersCoinList = allCoins.filter(coin => finalRemainingTickers.includes(coin.symbol));
            const usersCoinIds = usersCoinList.map(coin => coin.id);
            ...
            if (usersCoinIds.length > 0) {
              ...
              if (priceRes.ok) {
                const usersMarketData = await priceRes.json();

                finalRemainingTickers.forEach(t => {
                  try {
                    const meta = usersCoinList.find(c => c.symbol === t);
                    if (!meta) return;
                    const dataForId = usersMarketData[meta.id];
      ...
      console.warn('Enhanced market data error:', e);
      this.setState({ marketData: {} });
    }
  };
  ...
  setupAlertMonitoring = async () => {
    ...
    try {
      await enhancedAlertManager.initialize();

      // Set up event listener for triggered alerts
      window.addEventListener('alertTriggered', this.handleEnhancedAlertTriggered);

      console.log('Enhanced alert system initialized');
    } catch (error) {
      console.error('Failed to initialize enhanced alert system:', error);
    }

    // Set up alert monitoring with the legacy system as backup
    alertMonitor.addListener(this.handleTriggeredAlerts);
    alertMonitor.start(this.state.marketData, this.state.exchangeRates[this.state.pref.currency] || 1);
  };
  ...
  handleTriggeredAlerts = (triggeredAlerts) => {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      triggeredAlerts.forEach(alert => {
        new Notification(`🚨 ${alert.coinSymbol} Alert Triggered`, {
          body: `${alert.coinSymbol} price is now $${alert.currentPrice.toFixed(2)}, ${alert.alertType} your alert of $${alert.targetPrice.toFixed(2)}`,
          icon: '/favicon.ico'
        });
      });
    }
  ...
  handleEnhancedAlertTriggered = (event) => {
    const alert = event.detail;
    console.log('Enhanced alert triggered:', alert);

    // Update state with the triggered alert
    this.setState(prevState => ({
      triggeredAlerts: [...prevState.triggeredAlerts, alert]
    }));
  };

  // Update portfolio data in alert manager
  updateAlertManagerPortfolio = () => {
    const portfolioData = {
      totalValue: this.state.totalPortfolio?.totalValue || 0,
      totalGainLoss: this.state.totalPortfolio?.totalGainLoss || 0,
      gainLossPercent: this.state.totalPortfolio?.gainLossPercent || 0,
      coinz: this.state.coinz,
      currency: this.state.pref.currency
    };

    enhancedAlertManager.updatePortfolioData(portfolioData);
  };

  componentWillUnmount() {
    // Clean up alert monitoring
    alertMonitor.removeListener(this.handleTriggeredAlerts);
    alertMonitor.stop();

    // Clean up enhanced alert manager
    window.removeEventListener('alertTriggered', this.handleEnhancedAlertTriggered);
    enhancedAlertManager.destroy();
  }
  ...
  initializeSampleNotifications = () => {
    ...
    notificationManager.addSystemNotification(
      "🎉 Welcome to CoinFox",
      "Your portfolio tracking and alert system is now active!"
    );

    // Add a portfolio setup notification
    notificationManager.addSystemNotification(
      "📊 Portfolio Setup",
